...
class Streaming(object):
    ...
    def listen(self,track=None,location=None,users=None,follow=None):
        streamer = tweepy.Stream(auth=self.auth1,listener=self.listener,timeout=300000000000)
        streamer.filter(follow=follow,track=track,locations=location)

    def sample(self):
        stream = tweepy.Stream(auth=self.auth1, listener=self.listener)
        stream.sample()
